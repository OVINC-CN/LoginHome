...
const getPageTitle = (pathname: string, t: Record<string, string>): string => {
    ...
    if (pathname.startsWith('/login')) {
        return t.LoginToOVINC;
    }
    if (pathname.startsWith('/reset-password')) {
        return t.ResetPassword;
    }
    if (pathname.startsWith('/agreement/user')) {
        return t.UserAgreement;
    }
    if (pathname.startsWith('/agreement/privacy')) {
        return t.PrivacyAgreement;
    }
